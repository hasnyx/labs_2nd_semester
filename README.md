# Laboratory Work 3, Variant 3, Level 3
...
```
Додаткове завдання: Зчитуй дані (read_inorder): Витягуй список елементів із текстового файлу task.txt для їх подальшої обробки.

Будуй дерево (build_from_inorder): Створюй збалансоване бінарне дерево пошуку. Призначай центральний елемент списку коренем, а ліву та праву частини рекурсивно перетворюй на відповідні піддерева.

Обчислюй висоту (height): Визначай загальну кількість рівнів у структурі дерева, щоб точно розрахувати розміри графічного полотна.

Візуалізуй структуру (_build_canvas): Геруй «карту» дерева у текстовому форматі. Використовуй символи (-, /, \) для наочного позначення зв’язків між вузлами та виводь результат на екран.

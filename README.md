# Laboratory Work 1, Variant 3, Level 3
...
Додаткове завдання:
Створити прикладну систему аналізу тренувань для спортсмена (велосипедиста), яка перетворює абстрактний алгоритм на інструмент для реального світу. Програма обробляє масив щохвилинної потужності (Power) і знаходить найдовший правильний цикл, що складається зі строгого зростання (розігрів), точки максимуму (пік) та строгого спадання (відновлення). Технічно ви реалізували це через два модулі: файл longest_peak.py виконує роль обчислювальної бібліотеки, яка за лінійний час $O(n)$ знаходить тривалість циклу та конкретну хвилину вершини, а файл cycle.py забезпечує інтерфейс для введення даних користувачем через input() та виведення зрозумілого звіту. Ви оптимізували код, замінивши функцію max() на прозору конструкцію if-else та використавши константи, що робить програму надійною, легкою для аудиту та зручною для кінцевого користувача.

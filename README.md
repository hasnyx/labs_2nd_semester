...
Додаткове завдання:
Створити прикладну систему аналізу тренувань для велосипедиста. Програма обробляє масив щохвилинної потужності і знаходить найдовший правильний цикл, що складається зі строгого зростання, точки максимуму (пік) та строгого спадання. Реалізувати це через два модулі: файл longest_peak.py виконує роль обчислювальної бібліотеки, яка за лінійний час $O(n)$ знаходить тривалість циклу та конкретну хвилину вершини, а файл cycle.py забезпечує інтерфейс для введення даних користувачем через input() та виведення зрозумілого звіту.
